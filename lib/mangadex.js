...
// Build the thumbnail URL for a manga's cover_art relationship (needs the
// cover_art object to be included on the request via `includes[]=cover_art`).
function coverUrl(id, relationships) {
  const cover = relationships?.find((r) => r.type === "cover_art");
  const file = cover?.attributes?.fileName;
  return file ? `https://uploads.mangadex.org/covers/${id}/${file}.256.jpg` : null;
}

const pickText = (obj) =>
  obj ? obj.en || obj[Object.keys(obj)[0]] || "" : "";

export async function getManga(mangaId) {
  const { data } = await apiGet(`/manga/${mangaId}?includes[]=cover_art`);
  const attr = data.attributes;
  return {
    id: data.id,
    title: pickText(attr.title) || "Untitled",
    description: pickText(attr.description),
    year: attr.year,
    status: attr.status,
    cover: coverUrl(data.id, data.relationships),
  };
}

// Search titles for the unified cross-source search. Returns lightweight
// records (with cover + preview info) tagged with this source's id.
export async function searchManga(query, limit = 15) {
  ...
  return data.map((m) => ({
    source: id,
    id: m.id,
    title: pickText(m.attributes.title) || "Untitled",
