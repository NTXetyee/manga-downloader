...
//
// Dynasty exposes a convenient read-only JSON API by appending ".json" to any
// page URL:
//   • Series:  GET /series/<permalink>.json
//              -> { name, taggings: [ {header} | {permalink,title,released_on} ] }
//   • Chapter: GET /chapters/<permalink>.json
//              -> { title, tags, pages: [ { url: "/system/…jpg" } ] }
// Page image `url`s are site-relative, so we resolve them against BASE.
//
...
export async function getManga(id) {
  ...
  if (kind === "chapter") {
    ...
    return { id, title: data.title || permalink, description: "", status: "" };
  }
...
export async function getChapterPageUrls(chapterId, _opts = {}) {
  const data = await apiGet(`/chapters/${chapterId}.json`);
  const pages = Array.isArray(data.pages) ? data.pages : [];
  return pages.map((p) => new URL(p.url, BASE).href);
}
